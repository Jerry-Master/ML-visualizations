// the margin for the left side of the screen
let margin = 4*25;
// this is used to move up the network
let moved = 50;
...
function neuron_pos(layer, i, arch) {
  let n = arch.length;
  let hor_dist = width / (2*(n));
  let x = layer * hor_dist + margin;

  let num_neuron = arch[layer];
  let ver_dist = height / (num_neuron+1);
  let y = i * ver_dist -moved;
...
function draw_nn(arch){
  ...
  let hor_dist = width / (2*(n));
  for (let i = 0; i < n; i++){
    draw_layer(arch[i], i * hor_dist + margin, 2*rad);
  }
  // Draw weights
  for (let i = 0; i < n-1; i++){
...
function draw_layer(num_neur, x_pos, diam) {
  ...
  for (let i = 1; i <= num_neur; i++) {
    fill(255, 255, 255);
    stroke(0);
    circle(x_pos, ver_dist * i - 50, diam);
  }
}
...
function draw_connections(i, j, ni, nj, rad, arch) {
  for (let ii = 1; ii <= ni; ii++) {
    for (let jj = 1; jj <= nj; jj++) {
      ...
      let neuron_to = jj-1;
      */
      let alpha = 1; // original: curr_layer[neuron_to][neuron_from];
      let scale = 200;
